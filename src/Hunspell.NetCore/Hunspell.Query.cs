...
namespace Hunspell
{
    public partial class Hunspell
    {
        private class Query
        {
            public Query(string word, Hunspell hunspell)
            {
                wordToCheck = word;
            ...
            private readonly string wordToCheck;

            private readonly Hunspell hunspell;

            private AffixConfig Affix
            {
            ...
            {
                return new Query(word, hunspell).Check();
            }

            private SpellCheckResult CheckDetails(string word)
            {
                return new Query(word, hunspell).CheckDetails();
            }
            ...
            {
                return new Query(word, hunspell).Suggest();
            }
            ...
            private WordEntry CheckWord(string w, ref SpellCheckResultType info, out string root)
            {
                root = null;
                string w2;
                string word;
            ...
            private WordEntry CompoundCheck(string word, int wordNum, int numSyllable, int maxwordnum, int wnum, Dictionary<int, WordEntry> words, ref Dictionary<int, WordEntry> rwords, bool huMovRule, int isSug, ref SpellCheckResultType info)
            {
            ...
            /// </summary>
            private WordEntry PrefixCheck(string word, CompoundOptions inCompound, FlagValue needFlag)
            {
                ClearPrefix();
                PrefixAppend = null;
                SuffixAppend = null;
            ...
            private WordEntry PrefixCheckTwoSfx(string word, CompoundOptions inCompound, FlagValue needFlag)
            {
                ClearPrefix();
                SuffixAppend = null;
                SuffixExtra = 0;
            ...
            private WordEntry SuffixCheck(string word, AffixEntryOptions sfxOpts, AffixEntryWithDetail<PrefixEntry> pfx, FlagValue cclass, FlagValue needFlag, CompoundOptions inCompound)
            {
                WordEntry rv;
            ...
            /// </summary>
            private WordEntry SuffixCheckTwoSfx(string word, AffixEntryOptions sfxopts, AffixEntryWithDetail<PrefixEntry> pfx, FlagValue needflag)
            {
                WordEntry rv;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
            private WordEntrySet Lookup(string word)
            {
