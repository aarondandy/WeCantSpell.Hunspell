...
internal sealed class ArrayBuilder<T> : IList<T>
{
    ...
    internal bool AddAsSortedSet(T value)
    {
        if (_count == 0)
        {
            Add(value);
    ...
    internal void AddAsSortedSet(IEnumerable<T> values)
    {
        var expectedCount = values.GetNonEnumeratedCountOrDefault();
        if (expectedCount > 0)
        {
            EnsureCapacity(_count + expectedCount);
        }
    ...
    internal void AddAsSortedSet(ReadOnlySpan<T> values)
    {
        AddRange(values);
        Array.Sort(_values, 0, _count);
        RemoveAdjacentDuplicates();
